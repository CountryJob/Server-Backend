...
from models.train_multi_model import WorkforceWagePredictor
import re
import math
import sys
import os
...
model_pipeline = None
model_files = [
    "ai/models/work_type_aware_model.pkl"
]
...
def process_job_posting(title, area_size, start_date, end_date, work_time, duration_days, verbose=True):
    ...
    # 결과 정리
    result = {
        'original_work_type': title,
        'matched_work_type': workforce_result['matched_work_type'],
        'match_reason': workforce_result['match_reason'],
        'description': description,
        'male_count': workforce_result['male_count'],
        'female_count': workforce_result['female_count'],
        'wage_male': workforce_result['wage_male'],
        'wage_female': workforce_result['wage_female'],
        'total_count': workforce_result['male_count'] + workforce_result['female_count'],
        'characteristics': workforce_result['characteristics'],
        'female_ratio': workforce_result['female_ratio'],
        'work_analysis': analyze_work_type(workforce_result)
    }
...
if __name__ == "__main__":
    ...
    test_cases = [
        # 포장 중심 작업
        ("상추 포장 작업", 50.0, date(2025,8,10), date(2025,8,12), "08:00~17:00", 3),
        ("토마토 선별 포장", 80.0, date(2025,8,10), date(2025,8,12), "08:00~17:00", 3),
        ("배 봉지 씌우기", 100.0, date(2025,7,1), date(2025,7,3), "08:00~16:00", 3),
        
        # 체력 중심 작업
        ("나무 상차 작업", 200.0, date(2025,7,15), date(2025,7,17), "07:00~18:00", 3),
        ("벼 포대 운반", 150.0, date(2025,9,1), date(2025,9,3), "06:00~17:00", 3),
        
        # 혼합 작업
        ("상추 수확", 50.0, date(2025,8,10), date(2025,8,12), "08:00~17:00", 3),
        ("고추 따기", 80.0, date(2025,8,5), date(2025,8,7), "08:00~16:00", 3),
        ("잔디 심기", 120.0, date(2025,4,1), date(2025,4,3), "08:00~17:00", 3),
        
        # 세밀한 작물
        ("블루베리 수확", 60.0, date(2025,7,10), date(2025,7,12), "07:00~16:00", 3),
        ("복분자 따기", 1300, date(2025,6,15), date(2025,6,17), "07:00~15:00", 3),
    ]
    
    print("=== 통합 인력-임금 예측 테스트 ===\n")
    
    for i, (title, area, start_date, end_date, work_time, duration) in enumerate(test_cases, 1):
        print(f"【테스트 {i}】")
        result = process_job_posting(title, area, start_date, end_date, work_time, duration)
        
        print(f"\n작업 내용:")
        print(result['description'])
        
        print(f"\n예상 인력: {result['work_analysis']}")
        print(f"   남성 {result['male_count']}명, 여성 {result['female_count']}명")
        
        print(f"\n예상 임금:")
        if result['wage_male'] > 0:
            print(f"   남성 임금: {result['wage_male']:,}원")
        if result['wage_female'] > 0:
            print(f"   여성 임금: {result['wage_female']:,}원")
        
        # 작업 특성 출력
        if result['characteristics']:
            print(f"\n감지된 특성:")
            for char in result['characteristics']:
                print(f"   - {char['type']}: {char['keywords']}")
        
        print(f"\n여성 비율: {result['female_ratio']:.1%}")
